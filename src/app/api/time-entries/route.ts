...
type ActorContext = { userId: string; role: string | null; companyId: string | null; employeeId: string | null };
...
async function getActorContext(supabase: Awaited<ReturnType<typeof createServerSupabase>>) {
  ...
  return { userId: user.id, role: role?.role ?? null, companyId: role?.company_id ?? employee?.company_id ?? null, employeeId: employee?.id ?? null } satisfies ActorContext;
}

function isAdmin(actor: ActorContext) { return actor.role === "admin"; }
